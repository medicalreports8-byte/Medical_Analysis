...
def analyze_report(api_key, text=None, file_bytes=None, file_type=None):
...
with st.sidebar:
    ...
    st.code("pip install streamlit anthropic PyMuPDF Pillow pytesseract", language="bash")
    st.markdown("**▶️ Run:**")
    st.code("streamlit run app.py", language="bash")

# Resolve API key: sidebar input takes priority, fallback to secrets
def get_api_key():
        ...
        try:
            ...
            if uploaded:
                ...
                if uploaded.type == "text/plain":
                    result = analyze_report(api_key, text=fbytes.decode("utf-8"))
                else:
                    result = analyze_report(api_key, file_bytes=fbytes, file_type=uploaded.type)
            elif report_text.strip():
                result = analyze_report(api_key, text=report_text)
            else:
